...
export const works: Work[] = [
  ...
  */
  ...
  },
  {
    ...
    approach: "ゲームエンジンや操作はデスカマーと共通のまま、世界観をまるごと子ども向けに置き換え。たてにふる＝つよいこうげき／よこにふる＝まわりをこうげき／とまる＝てきのこうげきをはねかえす／あるく＝すきにうごく、と遊び方はそのままに、敵をかわいいスライム・おばけへ、アイテムを“いちごジュース（げんきかいふく）”“ほしのクリスタル（パワーアップ）”へ、UI・演出の文字をすべてひらがな・カタカナへ差し替えた。ステージ1＝じかんないにたくさんたおそう、ステージ2＝たまをはねかえして4たいたおそう、ステージ3＝おばけキングをたおせ、の3段構成で、ボスの“おばけキング”はとまってシールドではねかえすとダメージが入る。難易度もやさしく再調整。ゲーム本体はクリアまで動作し、M5StickやLiDARが無くてもキーボードで体験できる。",
    tools: ["C++11 / OpenGL / GLUT", "M5StickC Plus2", "LiDAR"],
  ...
  },
  {
    ...
    approach: "ポートフォリオ本体とは世界観を切り替え、クリーム色×パステルの“可愛い別世界”を全面設計。チェキ・マスキングテープ・付箋・方眼ノートを共通部品化し、全ページで一貫した「推し活ノート」の手触りに。線で結ぶ人物相関図、チケット半券風のライブ年表、名場面コラージュ、写真のライトボックス、誕生日カウントダウン、隠しハート集めなど「めくって楽しい」仕掛けを実装。情報は読みやすく整理しつつ、推し活ならではの高揚感を表現した。",
    slideGroups: [
  ...
  },
  {
    ...
    approach: "5人チームでの開発で、自分はUI・フロントエンドを中心に、レシートのAI読み取りなどのバックエンドや発表・登壇も担当した。機能は、①レシートを撮ると Gemini（2.5 Flash）のOCRが店名・金額・品目を自動入力、②全員で均等・金額を指定・商品ごとに支払う人を選ぶ、の3方式の割り勘、③食事・カフェ・交通などをアイコンで分類、④“誰が誰にいくら払うか”を最小回数の送金で求める自動精算、⑤招待コードでのイベント・参加者管理、⑥フレンド申請と取引履歴、⑦承認・催促・拒否のアプリ内通知、⑧イベントごとの精算進捗（未精算の残り・完了率）の可視化、までをカバーした。技術は Vue 3（script setup）＋Vite、ルーティングは Vue Router、バックエンドは Firebase（Authentication／Firestore／Cloud Functions／Hosting／FCM）で、精算ロジックは composable に切り出している。APIキーはサーバーレス関数側に置いてフロントには出さず、スマホのスワイプUIとPCレスポンシブの両対応をめざした。",
    tools: ["Vue 3 / Vite", "Firebase", "Gemini 2.5 Flash（OCR）"],
  ...
  {
    slug: "portfolio-vol1",
    title: "ポートフォリオ vol.1（SPECTRUM）",
    category: "web",
    summary: "このサイトの前作。「つくって、光を放つ。」をテーマに、深い闇色×スペクトラム（可視光線）で作ったポートフォリオ。自分の配色作品（可視光線・加法／減法混色のポスター）から着想した“光と色”を全面に使い、いろいろな分野に手を出すほど出せる色が増える、を視覚化した。いまの vol.2 と見比べられるよう、そのまま公開して残している。Astro 製。",
    goal: "「いろいろなことに手を出すほど、出せる色が増えていく」を、光のスペクトラムで表現する。量産型に見えない、映画の予告編のような没入感のあるポートフォリオをめざした。",
    approach: "深い闇色の背景に、可視光線のスペクトラムを差し色として効かせる配色。巨大な日本語タイポ、フィルムのようなグレイン（粒状）テクスチャ、テーマに追従するカスタムカーソル、スクロールで要素が立ち上がる演出、初回だけのシネマティックなイントロなどを作り込んだ。Web・画像処理・CG・映像・企画・グラフィック・サウンドの全7分野を、分野ごとに色分けして一覧にした。デザインから実装（Astro／TypeScript／CSS）まで一人で担当。vol.2 では方向性を“明るい仕事場”へ切り替えたため、対照として見比べられるよう別サイトとして残している。",
    tools: ["Astro", "TypeScript", "CSS"],
    thumb: "/assets/works/portfolio-vol1.jpg",
    gallery: ["/assets/works/portfolio-vol1.jpg", "/assets/works/portfolio-vol1-works.jpg", "/assets/works/portfolio-vol1-about.jpg"],
    status: "前作・公開中",
    link: "https://ryoo-ma638.github.io/",
  },

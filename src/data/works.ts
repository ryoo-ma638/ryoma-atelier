...
export const works: Work[] = [
  ...
  */
  ...
  // ====================== 画像処理・インタラクション（研究室＝CG・画像処理・インタラクションに直結） ======================
  {
    ...
    approach: "床のLiDARで足の位置を、鎌に付けたM5StickC（6軸センサー）で振りを検知。歩く＝移動＋向き、縦振り（A）＝射程の長い強攻撃、横振り（S）＝範囲攻撃、立ち止まる＝防御（前方の弾を反射し、反射で攻撃もできる）。WAVE1→2→3（ボス）→クリアまで通して遊べ（WAVE1＝制限時間内に多く倒す／WAVE2＝弾を反射して敵を4体倒す／WAVE3＝巨大ボス討伐。回復の薬でHP＋50、力の宝珠で攻撃力1.5倍。ボスは連射弾・溜めビーム・全方位爆発・近接衝撃波で攻めてくるが、溜め動作中に攻撃を当てると技をキャンセルできる）、床面にLiDARの点群・壁面に戦闘画面を映す2画面構成をめざした。企画・ハード・C++11/OpenGLの描画まで個人制作。デジコン演習(2025)で発表したのち“動かない原因はハードではなくコードの競合だった”と精読で突き止め、水野慎士研究室の個人プロジェクトとして再設計・継続開発中（向き制御の二重化や防御判定の競合を修正し、M5StickやLiDARが無くてもキーボードで通しプレイできる状態まで整備）。その後、WAVE突入時の攻撃封印／解放の演出、踏むと遊び方が全画面で出るヘルプ、足元に自分の位置を示すマーカー、日本語のHUD、クリア時のランク・スコア評価（最高ランクS）を追加し、GitHubで公開した。",
